...
# Fetch only news articles where the relevance score is NULL
...
for headline, text, rowid in news_articles:
    try:
        # Prepare the prompt with the article's headline and text
        prompt = prompt_template.format(headline=headline, text=text)

        # Generate relevance using the Llama model
        output = llm(
            prompt,
            max_tokens=2000,
            stop=["</s>"],
        ...
        # In case of any error (such as the article being too long), set relevance to NULL and continue
        cursor.execute("UPDATE news SET relevance = NULL WHERE rowid = ?", (rowid,))
        print(f"Error processing article with rowid {rowid}: {e}")

    # Commit changes after each article
    conn.commit()
